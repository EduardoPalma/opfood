...
def convert_food_portions(food_portions):
    data_dict_portions = [{"name": translate_single(portions.get("measureUnit").get("name")),
                           "abbreviation": translate_single(portions.get("measureUnit").get("abbreviation")) ,
                           "grams": float(portions.get("gramWeight")), "amount": float(portions.get("amount"))} for
...
def transform_data_in_document(data_json, db_instance: Database[Mapping[str, Any]]):
    ...
    for food_information in data_json['FoundationFoods']:
...
if __name__ == '__main__':
    ...
    data = read_json_food_data_central(path / "files_utils/foundationDownload.json")
